...
class RecordingFrame(ctk.CTkFrame):
    ...
    def set_recording_ui_parts(self):
        ...
        self.recording_lb = ctk.CTkLabel(self, width=75, height=20, font=("Cosmic Sans MS", 18, "normal"), text="Record", )
        ...
        # destination path
        self.dest_dir = ctk.CTkButton(self.f3b, width=75, height=20, text="Dest dir", command=lambda: self.select_path(1), )
        self.dest_dir.grid(row=1, column=0, padx=(5, 5), pady=(5, 5))

        self.dest_dir_entry = ctk.CTkEntry(self.f3b, width=200, height=20, justify="center")
        self.dest_dir_entry.grid(row=1, column=1, columnspan=3, padx=(5, 5), pady=(5, 5))
        self.dest_dir_entry.insert(0, "")

        # recording settings
        self.nbr_of_records_lb = ctk.CTkLabel(self.f3b, width=50, height=20, text="Nbr Records")
        self.nbr_of_records_lb.grid(row=2, column=0, padx=(5, 5), pady=(5, 5))       
        self.nbr_of_records = ctk.CTkEntry(self.f3b, width=50, height=20, justify="center")
        self.nbr_of_records.grid(row=2, column=1, padx=(5, 5), pady=(5, 5))
        self.nbr_of_records.insert(0, "42")

        self.delay_between_rec_lb = ctk.CTkLabel(self.f3b, width=50, height=20, text="Delay BScans")
        self.delay_between_rec_lb.grid(row=2, column=2, padx=(5, 5), pady=(5, 5))

        self.delay_between_rec = ctk.CTkEntry(self.f3b, width=50, height=20, justify="center")
        self.delay_between_rec.grid(row=2, column=3, padx=(5, 5), pady=(5, 5))
        self.delay_between_rec.insert(0, "0")

        self.nbr_of_bscans_lb = ctk.CTkLabel(self.f3b, width=50, height=20, text="Nbr BScans")
        self.nbr_of_bscans_lb.grid(row=3, column=0, padx=(5, 5), pady=(5, 5))

        self.nbr_of_bscans = ctk.CTkEntry(self.f3b, width=50, height=20, justify="center")
        self.nbr_of_bscans.grid(row=3, column=1, padx=(5, 5), pady=(5, 5))
        self.nbr_of_bscans.insert(0, "100")

        # control buttons
        self.start_rec = ctk.CTkButton(self.f3b, width=75, height=20, text="Start Rec", command=self.start_recording, )
        self.start_rec.grid(row=4, column=0, padx=(5, 5), pady=(5, 5))

        self.current_record_lb = ctk.CTkLabel(self.f3b, width=50, height=20, text="Current Rec")
        self.current_record_lb.grid(row=4, column=2, padx=(5, 5), pady=(5, 5))

        self.current_record = ctk.CTkLabel(self.f3b, width=50, height=20, text="0 of 42")
        self.current_record.grid(row=4, column=3, padx=(5, 5), pady=(5, 5))

        self.stop_rec = ctk.CTkButton(self.f3b, width=75, height=20, text="Stop Rec", command=self.stop_recording)
