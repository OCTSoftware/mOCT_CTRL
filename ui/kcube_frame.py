...
class KcubeFrame(ctk.CTkFrame):
    def __init__(self, parent, ctrl, config):
        ...
        self.kcube_bt_p100 = ctk.CTkButton(
            ...
            height=20,
            text="+100 µm",
            command=lambda: self.move_relative_and_refresh(0.1),
        )

        self.kcube_bt_p100.grid(row=0, column=0, padx=5, pady=5)

        self.kcube_bt_p50 = ctk.CTkButton(
            self.kcube_f3,
            width=75,
            height=20,
            text="+50 µm",
            command=lambda: self.move_relative_and_refresh(0.05),
        )

        self.kcube_bt_p50.grid(row=1, column=0, padx=5, pady=5)

        self.kcube_bt_p10 = ctk.CTkButton(
            self.kcube_f3,
            width=75,
            height=20,
            text="+10 µm",
            command=lambda: self.move_relative_and_refresh(0.01),
        )
        ...
        self.kcube_bt_m10 = ctk.CTkButton(
            self.kcube_f3,
            width=75,
            height=20,
            text="-10 µm",
        ...
        self.kcube_bt_m50 = ctk.CTkButton(
            self.kcube_f3,
            width=75,
            height=20,
            text="-50 µm",
        ...
        self.kcube_bt_m100 = ctk.CTkButton(
            self.kcube_f3,
            width=75,
            height=20,
            text="-100 µm",
        ...
        self.kcube_lb_position = ctk.CTkLabel(
            self.kcube_f4, width=75, height=20, text=""
        )

        self.kcube_lb_position.grid(row=0, column=0, padx=5, pady=5)

        self.kcube_tf_position = ctk.CTkEntry(
            self.kcube_f4,
            width=75,
            height=20,
            justify="center",
            placeholder_text="pos / mm",
        )

        self.kcube_tf_position.grid(row=1, column=0, padx=5, pady=5)
        self.kcube_tf_position.insert(0, "0")

        self.kcube_bt_go = ctk.CTkButton(
            self.kcube_f4,
            width=75,
            height=20,
            text="Go",
        ...
        self.kcube_cb_speed = ctk.CTkComboBox(
            self.kcube_f5, width=75, height=20, values=["slow", "medium", "fast"]
        )

        self.kcube_cb_speed.grid(row=0, column=0, padx=5, pady=(100, 5))

        self.kcube_bt_enable = ctk.CTkButton(
            self.kcube_f5, width=75, height=20, text="Enable"
        )

        self.kcube_bt_enable.grid(row=1, column=0, padx=5, pady=5)

        self.kcube_bt_home = ctk.CTkButton(
            self.kcube_f5,
            width=75,
            height=20,
            text="Home",
            command=self.home_and_refresh,
        )

        self.kcube_bt_home.grid(row=2, column=0, padx=5, pady=5)

        self.kcube_cb_objective = ctk.CTkComboBox(
            self.kcube_f5,
            width=75,
            height=20,
            values=["home", "05x16", "10x03", "20x05", "40x08"],
            command=self.select_objective,
        )

        self.kcube_cb_objective.grid(row=3, column=0, padx=5, pady=5)

        self.kcube_lb_vers = ctk.CTkLabel(
            self.kcube_f5,
            width=75,
            height=20,
            font=("Cosmic Sans MS", 10, "normal"),
            text=self.kcube_sn_info,
        )
